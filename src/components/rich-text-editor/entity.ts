import { ContentState, ContentBlock, EditorState, Modifier, SelectionState, Entity, RichUtils } from "draft-js";
...
export class EditorEntity<D> {
     ...
     */
    create (contentState: ContentState): ContentState {
        let contentStateWithEntity = contentState.createEntity(this.type, this.mutability, this.data);
        return contentStateWithEntity;
    }

    /**
     * Returns a new contenstate where this entity has been toggle for the current selection (similar to the build in inline styles)
     * 4 possibilities
    ...
    public toggle (editorState: EditorState, callback: (editorState: EditorState) => void) {
        ...
        var block: ContentBlock = contentState.getBlockForKey(blockKey);
        ...
        if (endKey == blockKey && startOffset == endOffset) {
            ...
            if (entityKey == null) {
                ...
                var prevSpace = text.lastIndexOf(' ', startOffset - 1);
                var nextSpace = text.indexOf(' ', startOffset);
                
                let nextContentState = this.create(contentState);
                let newEntityKey = nextContentState.getLastCreatedEntityKey();
                let wordSelection = selection.merge({
                    anchorKey: blockKey,
                    anchorOffset: prevSpace != -1 ? prevSpace + 1 : 0,
                    focusOffset: nextSpace != -1 ? nextSpace : text.length,
                    focusKey: blockKey,
                }) as SelectionState
                let newContentState = Modifier.applyEntity(nextContentState, wordSelection, newEntityKey);
                return callback(EditorState.push(editorState, newContentState, "apply-entity"));
            } else {
                // Remove the entity
                block.findEntityRanges((value) => {
                    let key = value.getEntity();
                    return key != null && contentState.getEntity(key).getType() == this.type
                }, (start: number, stop: number) => {
                    if (start <= startOffset && stop >= endOffset) {
                        let entitySelection = selection.merge({
                            anchorKey: blockKey,
                            anchorOffset: start,
                            focusKey: blockKey,
                            focusOffset: stop,
            ...
            let selectedEntityKey = block.getEntityAt(startOffset);
            let toggleOn = selectedEntityKey == null || contentState.getEntity(selectedEntityKey).getType() != this.type;
            var newContentState: ContentState;
            if (toggleOn) {
                // Toggle the entity on
                let contentStateWithEntity = this.create(contentState);
                let newEntityKey = contentStateWithEntity.getLastCreatedEntityKey();
